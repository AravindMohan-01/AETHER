...
def report_alerts(scanid):
    ...
    # ✅ Paths to the Images
    left_logo_path = "aethers.png"   # 🔹 Change to actual path
    right_logo_path = "college.jpeg" # 🔹 Change to actual path

    # ✅ Create Image Objects with Proper Sizing
    left_logo = Image(left_logo_path, width=80, height=80)
    right_logo = Image(right_logo_path, width=130, height=80)

    # ✅ Table to Align Images (Left & Right)
    image_table = Table([[left_logo, "", right_logo]], colWidths=[100, 350, 100])  # Adjust spacing
    image_table.setStyle(TableStyle([
        ('ALIGN', (0, 0), (0, 0), 'LEFT'),   # Left-align first image
        ('ALIGN', (2, 0), (2, 0), 'RIGHT'),  # Right-align second image
        ('VALIGN', (0, 0), (-1, -1), 'TOP'), # Align to top
    ]))

    # ✅ Add Images Table to the Story (Top of PDF)
    story.append(image_table)
    story.append(Spacer(1, 0.3 * inch))  # Space after images

    # ✅ Add Report Title
    title = Paragraph('<font size="20" color="black"><b>AETHER API Vulnerabilities Report</b></font>', styleT)
    story.append(title)
    story.append(Spacer(1, 0.5 * inch))

    # ✅ Section Header
    section_title = Paragraph('<font size="14" color="darkblue"><b>Vulnerability Details</b></font>', styleB)
    story.append(section_title)
    story.append(Spacer(1, 0.3 * inch))

    fileName = f"{scanid}.pdf"
    ...
    pdf.build(story)  # ✅ Builds the final PDF
